...
from connector_health_freshness import ConnectorHealthEvidence, project_connector_health
...
class ConnectorHealthFreshnessTests(unittest.TestCase):
    ...
    def test_health_projection_can_never_upgrade_not_ready(self):
        out = project_connector_health(base(ready=False, issues=["permission-approval-required"], next_action="review-permissions"), evidence(), now=1100)
        self.assertFalse(out["ready"])
