...
def main() -> None:
    ...
    repo_a = "acme/repo-a"
    repo_b = "acme/repo-b"
    common = dict(chat_id="chat-a", project_id="proj-a", repo_scope=repo_a, builder_session_id="build-1", revision=1)
    ...
    other = store.issue(kind="preview", chat_id="chat-a", project_id="proj-b", repo_scope=repo_b, epoch=1,
    ...
    result = apply_project_epoch_transition(
        chat_id="chat-a", project_id="proj-a", repo_scope=repo_a,
    ...
    assert sessions.calls == [("chat-a", "proj-a", repo_a, 2)]
    assert context.calls == [("chat-a", "proj-a", repo_a, 2)]
    try:
        store.resolve(stale, expected_kind="preview", chat_id="chat-a", project_id="proj-a", repo_scope=repo_a,
                      current_epoch=2, builder_session_id="build-1")
        raise AssertionError("stale handle survived transition")
    except AccessDenied:
        pass
    assert store.resolve(current, expected_kind="preview", chat_id="chat-a", project_id="proj-a", repo_scope=repo_a,
                         current_epoch=2, builder_session_id="build-1")["epoch"] == 2
    assert store.resolve(other, expected_kind="preview", chat_id="chat-a", project_id="proj-b", repo_scope=repo_b,
    ...
    expect_error(lambda: apply_project_epoch_transition(
        chat_id="chat-a", project_id="proj-a", repo_scope=repo_a, previous_epoch=2, current_epoch=3,
