...
class RegistryTests(unittest.TestCase):
    def test_ready_requires_all_gates(self):
        ...
        self.assertFalse(good(connected=False, verified_auth_method=None).ready)
    ...
    def test_local_plugin_does_not_require_connected(self):
        p = good(plugin_id="openbuilder", name="OpenBuilder", kind="builder", auth_method="local",
                 connected=False, verified_auth_method=None)
        self.assertTrue(p.ready)

    def test_registry_order_is_stable(self):
        a = good(plugin_id="zeta", name="Zeta")
        b = good(plugin_id="alpha", name="Alpha", kind="skill", auth_method="none",
                 connected=False, verified_auth_method=None)
    ...
    def test_health_setup_and_verified_auth_values_fail_closed(self):
        ...
        with self.assertRaises(ValueError):
            good(verified_auth_method="magic").validate()
        for bad in (True, 0, -1, 2**31):
            with self.subTest(bad=bad), self.assertRaises(ValueError):
                good(verified_setup_version=bad).validate()

    def test_disconnected_remote_plugin_cannot_claim_verified_auth(self):
        with self.assertRaises(ValueError):
            good(connected=False, verified_auth_method="oauth").validate()

    def test_auth_method_migration_requires_reverification(self):
        migrated = good(auth_method="oauth", verified_auth_method="api-key")
        self.assertFalse(migrated.ready)
        self.assertIn("auth-method-verification-required", migrated.diagnostics())
        self.assertEqual(migrated.next_action, "connect")
        state = migrated.public_state()
        self.assertEqual(state["auth_method"], "oauth")
        self.assertEqual(state["verified_auth_method"], "api-key")
    ...
    def test_test_connection_is_explicit_for_key_id_or_local_auth(self):
        ...
        self.assertTrue(good(auth_method="api-key", verified_auth_method="api-key").public_state()["test_connection_available"])
        self.assertTrue(good(auth_method="id", verified_auth_method="id").public_state()["test_connection_available"])
        self.assertTrue(good(auth_method="local", connected=False, verified_auth_method=None).public_state()["test_connection_available"])

    def test_launch_notice_for_enabled_unresolved_setup(self):
        notices = build_launch_notices([good(connected=False, verified_auth_method=None)], now=1000)
    ...
    def test_auth_migration_produces_persistent_connect_notice(self):
        ...
        self.assertEqual(len(notices), 1)
        self.assertEqual(notices[0]["reason"], "auth-method-verification-required")
        self.assertEqual(notices[0]["action"], "connect")
    ...
    def test_launch_notice_clears_when_resolved_and_ignores_disabled_optional(self):
        ...
        self.assertEqual(build_launch_notices([good(enabled=False, connected=False, verified_auth_method=None)], now=1000), [])
        self.assertEqual(build_launch_notices([good(installed=False, connected=False, enabled=False, verified_auth_method=None)], now=1000), [])

    def test_dismissal_is_temporary_and_bounded(self):
        p = good(connected=False, verified_auth_method=None)
    ...
    def test_changed_problem_does_not_inherit_old_dismissal(self):
        auth_problem = good(connected=False, verified_auth_method=None)
    ...
    def test_auth_method_change_invalidates_old_dismissal(self):
        old = good(auth_method="api-key", verified_auth_method="oauth")
        old_notice = build_launch_notices([old], now=1000)[0]
        dismissal = {"github": {"until": 1100, "notice_state_id": old_notice["notice_state_id"]}}
        new = good(auth_method="oauth", verified_auth_method="api-key")
        notices = build_launch_notices([new], dismissal, now=1000)
        self.assertEqual(len(notices), 1)
        self.assertNotEqual(notices[0]["notice_state_id"], old_notice["notice_state_id"])
